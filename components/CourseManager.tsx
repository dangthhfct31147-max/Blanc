...
const CourseManager: React.FC = () => {
  ...
  const getLevelColor = (level: Course['level']) => {
    switch (level) {
      ...
      case 'Intermediate': return 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300';
      case 'Advanced': return 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300';
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 dark:text-slate-400 mt-1">Manage learning materials and instructors</p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-600 dark:text-slate-400">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
              {courses.map((course) => (
                <tr key={course.id} className="hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <img src={course.image} alt="" className="h-10 w-16 object-cover rounded-md" />
                      <div>
                        <p className="font-semibold text-gray-900 dark:text-slate-100">{course.title}</p>
                        <p className="text-xs text-gray-400 dark:text-slate-400">ID: {course.id}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-gray-900 dark:text-slate-100 font-medium">{course.instructor}</span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-medium text-gray-900 dark:text-slate-100">{course.rating}</span>
                      <span className="text-gray-400 dark:text-slate-400 text-xs">({course.reviewsCount})</span>
                    </div>
                  </td>
                    ...
                    <div className="relative action-dropdown inline-block text-left">
                      ...
                      {openActionId === course.id && (
                        <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-gray-100 dark:border-slate-800 z-50 overflow-hidden animate-fade-in-up origin-top-right">
                          <div className="py-1">
                            <button onClick={() => handleAction('view', course)} className="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 flex items-center gap-2 transition-colors">
                              <Eye size={16} className="text-gray-400 dark:text-slate-400" />
                              <span>View Details</span>
                            </button>
                            <button onClick={() => handleAction('edit', course)} className="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 flex items-center gap-2 transition-colors">
                            ...
                            <button onClick={() => handleAction('delete', course)} className="w-full text-left px-4 py-2.5 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 flex items-center gap-2 transition-colors">
                              <Trash2 size={16} />
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col">
            <div className="px-6 py-4 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-slate-800">
              <h3 className="font-semibold text-gray-900 dark:text-slate-100">{newTitle && courses.find(c => c.title === newTitle) ? 'Edit Course' : 'New Course'}</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 dark:text-slate-400 hover:text-gray-600 dark:hover:text-slate-200">✕</button>
            </div>
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Price (VND)</label>
                  <input
                    type="number"
                    className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100"
                    placeholder="0"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Difficulty Level</label>
                <select
                  title="Chọn độ khó"
              ...
              <div className="bg-emerald-50 dark:bg-emerald-900/30 p-4 rounded-lg border border-emerald-100 dark:border-emerald-800">
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-sm font-semibold text-emerald-800 dark:text-emerald-300">AI Syllabus Generator</label>
                ...
                {generatedSyllabus && (
                  <div className="mt-2">
                    <label className="block text-xs font-medium text-emerald-700 dark:text-emerald-300 mb-1">Generated Syllabus:</label>
                    <textarea
                      ...
                      className="w-full text-xs p-2 rounded border border-emerald-200 dark:border-emerald-800 focus:ring-1 focus:ring-emerald-500 outline-none bg-white dark:bg-slate-800 text-gray-700 dark:text-slate-300 h-32"
                    />
                  </div>
                )}
              </div>
            ...
            <div className="px-6 py-4 bg-gray-50 dark:bg-slate-800 flex justify-end gap-3 border-t border-gray-100 dark:border-slate-800">
              <button
