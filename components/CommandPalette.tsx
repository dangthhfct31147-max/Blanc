    ...
    return (
        <>
            ...
            <button
                ...
                className="hidden md:flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white/60 px-2.5 py-1.5 text-sm text-slate-400 backdrop-blur transition-all hover:border-slate-300 hover:bg-white hover:text-slate-600 hover:shadow-sm dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300 dark:hover:border-slate-600 dark:hover:bg-slate-800 dark:hover:text-slate-100"
            >
                <Search size={14} />
                <span className="max-w-28 truncate lg:max-w-32">{t('common.search' as any) || 'Tìm kiếm...'}</span>
                <kbd className="ml-1.5 hidden items-center gap-0.5 rounded-md border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] font-medium text-slate-400 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-300 2xl:flex">
                    <Command size={10} />K
                </kbd>
            </button>
            ...
            <button
                onClick={() => setOpen(true)}
                className="flex md:hidden items-center justify-center rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-800/70 dark:hover:text-slate-100"
                aria-label="Search"
            >
                <Search size={18} />
            </button>
                ...
                {open && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed inset-x-4 top-[12vh] z-[101] mx-auto max-w-xl overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl shadow-slate-900/10 sm:inset-x-auto dark:border-slate-700/80 dark:bg-slate-900 dark:shadow-slate-950/40"
                        >
                            {/* Search input */}
                            <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3 dark:border-slate-800">
                                ...
                                <input
                                    ...
                                    className="grow bg-transparent text-sm text-slate-800 placeholder:text-slate-400 outline-none dark:text-slate-100 dark:placeholder:text-slate-500"
                                    autoComplete="off"
                                    spellCheck={false}
                                />
                                {isLoading && (
                                ...
                                <kbd className="hidden items-center rounded-md border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] font-medium text-slate-400 sm:flex dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-300">
                                    ESC
                                </kbd>
                            </div>
                            ...
                            <div ref={listRef} className="max-h-[50vh] overflow-y-auto overscroll-contain px-2 py-2">
                                {items.length === 0 && query.length >= 2 && !isLoading && (
                                    <div className="py-12 text-center text-sm text-slate-400">
                                        Không tìm thấy kết quả cho "<span className="font-medium text-slate-500 dark:text-slate-300">{query}</span>"
                                    </div>
                                )}
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="min-w-0 grow">
                                                        ...
                                                        {item.subtitle && (
                                                            <div className="truncate text-xs text-slate-400 dark:text-slate-500">{item.subtitle}</div>
                                                        )}
                                                    </div>
                                                    {isActive && (
                            ...
                            {/* Footer hints */}
                            <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/50 px-4 py-2 text-[11px] text-slate-400 dark:border-slate-800 dark:bg-slate-800/60 dark:text-slate-500">
                                <div className="flex items-center gap-3">
                                    <span className="flex items-center gap-1">
                                        <kbd className="rounded border border-slate-200 bg-white px-1 py-0.5 font-mono dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">↑↓</kbd>
                                        di chuyển
                                    </span>
                                    <span className="flex items-center gap-1">
                                ...
                                <span className="flex items-center gap-1">
                                    <kbd className="rounded border border-slate-200 bg-white px-1 py-0.5 font-mono dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">ESC</kbd>
                                    đóng
                                </span>
                            </div>
                        </motion.div>
