...
const ContestList: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <section className="relative mb-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-primary-100/60 dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">
        <div className="absolute inset-0 bg-linear-to-br from-primary-50 via-white to-amber-50 opacity-90 dark:from-slate-950 dark:via-slate-900 dark:to-amber-950/30 dark:opacity-100" aria-hidden="true" />
        <div className="absolute -top-24 -right-16 h-48 w-48 rounded-full bg-primary-200/40 blur-3xl dark:bg-primary-700/20" aria-hidden="true" />
        <div className="absolute -bottom-28 -left-16 h-56 w-56 rounded-full bg-amber-200/50 blur-3xl dark:bg-amber-700/10" aria-hidden="true" />
        <div className="relative p-6 md:p-8 lg:p-10">
          <div className="grid grid-cols-1 lg:grid-cols-[1.35fr_1fr] gap-8 items-center">
            <div className="space-y-4 animate-fade-in-up">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/80 border border-white/70 text-xs font-semibold text-primary-700 shadow-sm dark:border-primary-500/20 dark:bg-primary-500/10 dark:text-primary-200">
              ...
              <p className="text-sm md:text-base text-slate-600 leading-relaxed max-w-xl md:max-w-none md:whitespace-nowrap dark:text-slate-300">
                {t('contests.hero.description')}
              </p>
              <div className="flex flex-wrap gap-3">
                <div className="flex items-center gap-2 rounded-xl bg-white/90 border border-slate-100 px-3 py-2 text-xs text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300">
            ...
            <div className="space-y-4 animate-fade-in-up">
              <div className="rounded-2xl border border-white/80 bg-white/80 p-5 shadow-md dark:border-slate-700 dark:bg-slate-800/70 dark:shadow-none">
                <div className="text-xs font-semibold uppercase tracking-widest text-slate-400 dark:text-slate-500">{t('contests.stats.overview')}</div>
                <div className="mt-3 flex items-end gap-2">
                  <span className="text-3xl font-bold text-slate-900 dark:text-slate-100">{isLoading ? '--' : contests.length}</span>
                  <span className="text-sm text-slate-500 dark:text-slate-400">{t('contests.stats.contestNoun')}</span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-3">
                <div className="rounded-xl border border-emerald-100 bg-emerald-50/80 px-4 py-3 dark:border-emerald-500/20 dark:bg-emerald-500/10">
                  <div className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">{t('contests.stats.open')}</div>
                ...
                <div className="rounded-xl border border-amber-100 bg-amber-50/80 px-4 py-3 dark:border-amber-500/20 dark:bg-amber-500/10">
                  <div className="text-xs font-semibold text-amber-700 dark:text-amber-300">{t('contests.stats.upcoming')}</div>
                  <div className="mt-1 text-2xl font-bold text-amber-800 dark:text-amber-200">
                    {isLoading ? '--' : upcomingCount}
                  </div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white/80 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/70">
                  <div className="text-xs font-semibold text-slate-600 dark:text-slate-300">{t('contests.stats.ended')}</div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{t('contests.list.title')}</h1>
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <div className="relative grow md:w-64">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4 dark:text-slate-500" />
            <input
              type="text"
              placeholder={t('contests.search.placeholder')}
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full h-10 rounded-lg border border-slate-300 bg-white pl-9 pr-9 text-sm text-slate-900 focus:border-transparent focus:ring-2 focus:ring-primary-500 outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500"
            />
            {searchQuery && (
              <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <aside className={`${showFilters ? 'block' : 'hidden'} lg:block space-y-6`}>
          <div>
            <h3 className="mb-3 font-semibold text-slate-900 dark:text-slate-100">{t('contests.filters.fieldTitle')}</h3>
            <Dropdown
          ...
          <div>
            <h3 className="mb-3 font-semibold text-slate-900 dark:text-slate-100">{t('contests.filters.statusTitle')}</h3>
            <div className="space-y-2">
              {statusOptions.map((option) => (
                <label key={option.value} className="flex cursor-pointer items-center space-x-2 text-sm text-slate-600 dark:text-slate-300">
                  <input
                    type="checkbox"
          ...
          {isLoading ? (
              ...
              <Card key={i} className="animate-pulse">
                ...
                <div className="p-5">
                  <div className="flex justify-between mb-2">
                    ...
                    <div className="h-4 w-20 bg-slate-100 rounded dark:bg-slate-800" />
                  </div>
                  <div className="h-6 w-full bg-slate-200 rounded mb-2 dark:bg-slate-700" />
                  <div className="h-4 w-3/4 bg-slate-100 rounded mb-4 dark:bg-slate-800" />
                  <div className="flex gap-4 border-t border-slate-100 pt-4 dark:border-slate-800">
                    <div className="h-4 w-24 bg-slate-100 rounded dark:bg-slate-800" />
                    <div className="h-4 w-20 bg-slate-100 rounded dark:bg-slate-800" />
                  </div>
                </div>
              </Card>
            ...
            <>
              {paginatedContests.map((contest) => (
                <Card
                  key={contest.id}
                  className="flex h-full cursor-pointer flex-col hover:border-primary-200 dark:hover:border-primary-500/40"
                ...
                >
                  ...
                  <div className="p-5 flex flex-col grow">
                    <div className="flex items-center justify-between mb-2">
                      <span className="rounded border border-primary-100 bg-primary-50 px-2 py-0.5 text-xs font-semibold text-primary-600 dark:border-primary-500/20 dark:bg-primary-500/10 dark:text-primary-200">
                        {getCategoryLabel(contest.category || contest.tags?.[0] || 'General')}
                      </span>
                      <span className="text-xs text-slate-400 dark:text-slate-500">{getRemainingDays(contest.deadline)}</span>
                    </div>
                    <h3 className="mb-2 line-clamp-2 text-lg font-bold text-slate-900 dark:text-slate-100">{contest.title}</h3>
                    <p className="mb-4 grow line-clamp-2 text-sm text-slate-500 dark:text-slate-300">
            ...
            <div className="md:col-span-2 text-center py-12">
              ...
              <p className="mb-2 text-slate-500 dark:text-slate-300">{t('contests.empty.title')}</p>
              <p className="mb-4 text-sm text-slate-400 dark:text-slate-500">{t('contests.empty.subtitle')}</p>
              {hasActiveFilters && (
